...
- URL страницы (`meta.page.url`) и URL ресурсов (события `faro.performance.resource`) сокращаются до хоста
  и пути в нижнем регистре, а идентификаторы в пути заменяются на `:id`. Идентификаторами считаются UUID любой
  версии, в том числе с суффиксом, hex-строки от 12 символов и числа от 6 цифр.
